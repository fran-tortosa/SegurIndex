...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Actualiza placeholders solo si el elemento existe
    const updateAddFormPlaceholders = () => {
        const headers = segurosTable.querySelectorAll('thead th');
        [
            { elem: inputNombre, idx: 0 },
            { elem: inputOficina, idx: 1 },
            { elem: inputTipo, idx: 2 },
        ].forEach(({ elem, idx }) => {
            if (!elem) return;
            if (headers[idx]) {
                elem.placeholder = headers[idx].textContent.trim();
            } else {
                elem.placeholder = `Columna ${idx + 1} no disponible`;
            }
        });
    };
    
	// Ejecuta una vez al inicio para ajustar los placeholders
    updateAddFormPlaceholders();
});